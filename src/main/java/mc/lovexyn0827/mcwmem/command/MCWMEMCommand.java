						...
						then(argument("location",StringArgumentType.string()).suggests(suggest ).
								executes((ct)->{
									...
									if(newAlignMode==null) {
										CommandUtil.error(ct, "Undieined location:"+location);
										return -1;
									}
						...
						then(literal("setLifetime").
								then(argument("ticks",IntegerArgumentType.integer()).
										executes((ct)->{
											...
											MCWMEMod.INSTANCE.getExplosionRenderer().lifeTime = lifeTime;
								...
								executes((ct)->{
									if(FabricLoader.getInstance().isModLoaded("carpet")) {
										...
										MCWMEMod.INSTANCE.setOption("serverSyncedBox", 
											String.valueOf(bool));
										if(bool) {
											CommandUtil.execute(ct.getSource(),"/script load server_box global");
										}else {
											CommandUtil.execute(ct.getSource(),"/script unload server_box");
										}
										return 1;
									}else {
										CommandUtil.error(ct, NO_CARPET_ERROR);
