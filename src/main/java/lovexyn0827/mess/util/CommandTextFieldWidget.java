...
public class CommandTextFieldWidget extends TextFieldWidget {
	...
	protected MutableText getNarrationMessage() {
		return super.getNarrationMessage().append(this.commandSuggestor.get().method_23958());
	}
			...
			boolean isWordDelimiter(ArgumentCommandNode<?, ?> node, char ch, int pos) {
				...
				if (argType instanceof EntityArgumentType) {
					...
					return ch == ' ' || ch == '[' || ch == ',' || ch == '=' || ch == ':' || ch == ']';
						...
						|| argType instanceof IdentifierArgumentType || argType instanceof ParticleArgumentType) {
					// Identifier
					return ch == ' ' || ch == ':' || ch == '/';
				} else if (argType instanceof ItemPredicateArgumentType || argType instanceof ItemStackArgumentType
						|| argType instanceof NbtCompoundTagArgumentType || argType instanceof NbtTagArgumentType
					...
					// NBT path
					return ch == ' ' || ch == '.' || ch == '[' || ch == ']';
				} else if (argType instanceof ObjectiveCriteriaArgumentType) {
					// mc:a.b.c
					return ch == ' ' || ch == ':' || ch == '.';
