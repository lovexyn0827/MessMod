...
public final class Oscilscope {
	...
	public void update(ServerWorld world, BlockPos pos) {
		...
		if (this.channels.containsKey(key)) {
			...
			int color = world.getBlockState(pos.down()).getMapColor(world, pos).color | 0xFF000000;
	...
	Channel deserializeChannel(NbtCompound tag) {
	...
	private void uploadConfig(byte type, Consumer<PacketByteBuf> writer) {
		...
		CustomPayloadC2SPacket pkt = new CustomPayloadC2SPacket(
				new MessModPayload(Channels.OSCILSCOPE_CONF, buf));
		MessMod.INSTANCE.getClientNetworkHandler().send(pkt);
	}
	
	// Client to server
	public void handleConfigPacket(PacketByteBuf buf) {
		CustomPayloadS2CPacket broadcastPkt = new CustomPayloadS2CPacket(
				new MessModPayload(Channels.OSCILSCOPE_CONF_BROADCAST, 
