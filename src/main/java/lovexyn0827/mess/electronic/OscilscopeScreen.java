...
import lovexyn0827.mess.electronic.Oscilscope.TrigMode;
...
public final class OscilscopeScreen extends Screen {
	...
	public void init() {
		...
		this.modeBtn = ButtonWidget.builder(
						I18N.translateAsText(
				...
				.dimensions((int) (this.width * 0.5) - 90, (int) (this.height * 0.6) + 16, 
						80, 20)
				.build();
		this.addSelectableChild(this.modeBtn);
		this.tickAlignedBtn = CheckboxWidget.builder(I18N.translateAsText("oscil.gui.tickaligned"), textRenderer)
				.pos((int) (this.width * 0.5) + 10, (int) (this.height * 0.6) + 16)
				.checked(false)
				.build();
		this.addSelectableChild(this.tickAlignedBtn);
		...
		this.addDrawableChild(this.stroageDepth);
		this.resetBtn = ButtonWidget.builder(
		...
		.dimensions((int) (this.width * 0.5) + 10, (int) (this.height * 0.6) + 38, 
				80, 20)
		.build();
		this.addSelectableChild(this.resetBtn);
		...
		this.addDrawableChild(this.waveTransparancySlider);
	}
	...
	private void updateWaveArea() {
		...
		this.addDrawable(this.waveArea);
	...
	public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
		...
		this.trigHistory.render(ctx, mouseX, mouseY, delta);
		this.channelList.render(ctx, mouseX, mouseY, delta);
		this.modeBtn.render(ctx, mouseX, mouseY, delta);
		this.tickAlignedBtn.render(ctx, mouseX, mouseY, delta);
		this.stroageDepth.render(ctx, mouseX, mouseY, delta);
		this.resetBtn.render(ctx, mouseX, mouseY, delta);
		this.waveTransparancySlider.render(ctx, mouseX, mouseY, delta);
		this.waveArea.render(ctx, mouseX, mouseY, delta);
	}
	...
	public boolean mouseScrolled(double mouseX, double mouseY, double hAmount, double vAmount) {
		return super.mouseScrolled(mouseX, mouseY, hAmount, vAmount)
				|| this.getHoveringElement(mouseX, mouseY).mouseScrolled(mouseX, mouseY, hAmount, vAmount);
	}
	...
	private abstract class WaveArea implements Element, Drawable {
		...
		public boolean mouseScrolled(double mouseX, double mouseY, double hAmount, double vAmount) {
			if (Screen.hasControlDown()) {
				...
				this.tickPerDiv = this.calculateNextDivLength(this.tickPerDiv, vAmount < 0, 1000000);
				...
				this.leftTick += -(Screen.hasShiftDown() ? vAmount * this.hDivs : vAmount) * this.tickPerDiv;
			}
		...
		public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
			...
			this.drawGrids(ctx);
			this.drawAxises(ctx);
			this.drawTrigMarkers(ctx);
			this.drawWaves(ctx, mouseX, mouseY);
			this.drawStatus(ctx);
			this.drawToolTip(ctx, mouseX, mouseY);
		}

		protected void drawToolTip(DrawContext ctx, int mouseX, int mouseY) {
		}
		
		protected void appendToolTipIfNeeded(List<OrderedText> toolTip, int mouseX, int mouseY, 
			...
			if (edge != null && MathHelper.abs(mouseX - curX) <= 1 && mouseY > minY && mouseY < maxY) {
				...
				toolTip.add(Text.of(String.format("  CH%d: %d => %d", 
						channel.getId(), prevEdge.newLevel, edge.newLevel)).asOrderedText());
				...
				if (prevInvalid) {
					toolTip.add(Text.of("From: ?").asOrderedText());
				} else {
					toolTip.add(Text.of(String.format("  From: %d @ %s", 
							prevEdge.time.gameTime, prevEdge.time.phase)).asOrderedText());
				}
				...
				if (edge != null) {
					...
					toolTip.add(Text.of("  To: ?").asOrderedText());
				}
			}
		}

		protected abstract void drawStatus(DrawContext ctx);

		protected void drawBackground(DrawContext ctx) {
			ctx.fill(this.left, this.top, this.right, this.bottom, 0xFF000000);
		}

		protected abstract void drawWaves(DrawContext ctx, int mouseX, int mouseY);
		...
		private void drawTrigMarkers(DrawContext ctx) {
		...
		protected void drawTrigMarkers(Matrix4f matrix, BufferBuilder bufferBuilder) {
			int trigHorizontalPos = this.trigHDiv * this.pixelsPerHDiv + this.left;
			bufferBuilder.vertex(matrix, trigHorizontalPos, this.top, 0).color(1.0F, 1.0F, 0, 1.0F);
			bufferBuilder.vertex(matrix, trigHorizontalPos - 5, this.top - 5, 0).color(1.0F, 1.0F, 0, 1.0F);
			bufferBuilder.vertex(matrix, trigHorizontalPos + 5, this.top - 5, 0).color(1.0F, 1.0F, 0, 1.0F);
		}

		protected void drawAxises(DrawContext ctx) {
			int hStep = this.pixelsPerHDiv / VALID_TICKING_STAGE_COUNT;
			for (int i = 0; i <= this.hDivs * VALID_TICKING_STAGE_COUNT; i++) {
				ctx.drawVerticalLine(this.left + i * hStep, this.bottom, this.bottom - 2, 0xFFFFFFFF);
			}
			...
			for (int i = 0; i <= this.hDivs; i++) {
				...
				ctx.drawCenteredTextWithShadow(OscilscopeScreen.this.textRenderer, 
						Long.toString(this.getTickAtDiv(i)), 
						this.left + i * this.pixelsPerHDiv, this.bottom + 3, 0xFFFFFFFF);
				ctx.drawVerticalLine(this.left + i * this.pixelsPerHDiv, this.bottom, this.bottom - 3, 
						0xFFFFFFFF);
			}
		}
		...
		protected void drawGrids(DrawContext ctx) {
			for (int i = 0; i <= this.hDivs; i++) {
				ctx.drawVerticalLine(this.left + i * this.pixelsPerHDiv, this.top, this.bottom, 0x3FFFFFFF);
			}
		}
	...
	private final class AnalogWaveArea extends WaveArea {
		...
		public boolean mouseScrolled(double mouseX, double mouseY, double hAmount, double vAmount) {
			if (Screen.hasAltDown()) {
				if (Screen.hasControlDown()) {
					this.levelPerDiv = this.calculateNextDivLength(this.levelPerDiv, vAmount < 0, 100);
				} else {
					this.bottomLevel += (Screen.hasShiftDown() ? vAmount * this.vDivs : vAmount) * this.levelPerDiv;
				}
			} else {
		...
		protected void drawToolTip(DrawContext ctx, int mouseX, int mouseY) {
			if (this.draggingTrigLevelMarker != null) {
				OscilscopeScreen.this.setTooltip(
		...
		@Override
		protected void drawStatus(DrawContext ctx) {
			ctx.drawCenteredTextWithShadow(OscilscopeScreen.this.textRenderer, 
		...
		@Override
		protected void drawWaves(DrawContext ctx, int mouseX, int mouseY) {
			...
			synchronized (data) {
				drawSingleWave(ctx, mouseX, mouseY, toolTip, data);
			}
			
			if (!toolTip.isEmpty()) {
				OscilscopeScreen.this.setTooltip(toolTip);
			}
		}

		private void drawSingleWave(DrawContext ctx, int mouseX, int mouseY, List<OrderedText> toolTip,
				Map<Oscilscope.Channel, List<Oscilscope.Edge>> data) {
			data.forEach((channel, wave) -> {
				...
				for (Oscilscope.Edge edge : wave) {
					if (prevEdge != null) {
						...
						ctx.drawHorizontalLine(prevX, curX, prevY, color);
						ctx.drawVerticalLine(curX, prevY, curY, color);
				...
				ctx.drawHorizontalLine(this.calculatePosOfTime(prevEdge.time), 
		...
		protected void drawTrigMarkers(Matrix4f matrix, BufferBuilder bufferBuilder) {
			...
			OscilscopeScreen.this.waveData.getAllChannels().stream()
					...
					.forEach((yPos) -> {
						bufferBuilder.vertex(matrix, this.right, yPos, 0).color(1.0F, 1.0F, 0, 1.0F);
						bufferBuilder.vertex(matrix, this.right + 5, yPos - 5, 0).color(1.0F, 1.0F, 0, 1.0F);
						bufferBuilder.vertex(matrix, this.right + 5, yPos + 5, 0).color(1.0F, 1.0F, 0, 1.0F);
					});
		}

		protected void drawAxises(DrawContext ctx) {
			super.drawAxises(ctx);
		...
		protected void drawGrids(DrawContext ctx) {
			super.drawGrids(ctx);
			for (int i = 0; i <= this.vDivs; i++) {
				ctx.drawHorizontalLine(this.left, this.right, this.top + i * this.pixelsPerVDiv, 0x3FFFFFFF);
			}
		}
	}
	...
	private final class DigitalWaveArea extends WaveArea {
		...
		public boolean mouseScrolled(double mouseX, double mouseY, double hAmount, double vAmount) {
			if (Screen.hasAltDown()) {
				if (Screen.hasControlDown()) {
					this.waveHeight = MathHelper.clamp(
							vAmount > 0 ? this.waveHeight * 2 : this.waveHeight / 2, 4, 16);
	...
	private final class ChannelList extends CustomPositionEntryListWidget<ChannelList.Entry> {
		...
		private final class Entry extends AlwaysSelectedEntryListWidget.Entry<Entry> {
			...
			private final CyclingButtonWidget<TrigMode> trigMode;
			...
			public Entry(Oscilscope.Channel backend) {
				...
				this.trigMode = CyclingButtonWidget.builder(this::getTrigModeIndicator)
						.values(Oscilscope.TrigMode.values())
						.initially(this.backend.getTrigMode())
						.omitKeyText()
						.build(ChannelList.this.getRowLeft() + 23, 1, 20, 20, Text.empty(), (btn, mode) -> {
							this.backend.setTrigMode(mode);
							btn.setValue(mode);
						});
				this.trigLevel = new TextFieldWidget(OscilscopeScreen.this.textRenderer, 
						ChannelList.this.getRowLeft() + 47, 4, 
						20, 14, 
				...
				this.show = CheckboxWidget.builder(
								Text.of(I18N.translateAsText("oscil.gui.show").asTruncatedString(2)), textRenderer)
						.callback((box, checked) -> {
							Entry.this.backend.setVisible(checked);
						})
						.pos(ChannelList.this.getRowLeft() + 71, 1)
						.checked(this.backend.isVisible())
						.build();
				int lengthOfShow = ChannelList.this.client.textRenderer.getWidth(
						I18N.translateAsText("oscil.gui.show").asTruncatedString(2));
				this.active = CheckboxWidget.builder(
								Text.of(I18N.translateAsText("oscil.gui.active").asTruncatedString(2)), textRenderer)
						.callback((box, checked) -> {
							Entry.this.backend.setActiveUpdate(checked);
						})
						.pos(ChannelList.this.getRowLeft() + 100 + lengthOfShow, 1)
						.checked(this.backend.isUpdatingActively())
						.build();
			...
			private Text getTrigModeIndicator(Oscilscope.TrigMode mode) {
			...
			public void render(DrawContext ctx, int j, int y, int x, 
				...
				ctx.fill(x + 2, y + 2, x + 19, y + 19, 0xFFFFFFFF);
				ctx.fill(x + 3, y + 3, x + 18, y + 18, this.backend.getColor());
				this.trigMode.setY(y + 1);
				this.trigLevel.setY(y + 4);
				this.show.setY(y + 1);
				this.active.setY(y + 1);
				this.trigMode.render(ctx, mouseX, mouseY, var10);
				this.trigLevel.render(ctx, mouseX, mouseY, var10);
				this.show.render(ctx, mouseX, mouseY, var10);
				this.active.render(ctx, mouseX, mouseY, var10);
				int deltaX = mouseX - x;
				int deltaY = mouseY - y;
				if (deltaX >= 2 && deltaX <= 19 && deltaY >= 2 && deltaY <= 19) {
					Oscilscope.Channel ch = this.backend;
					ChannelList.this.tooltip = Lists.newArrayList(
							I18N.translateAsText("oscil.gui.chinf.1", ch.getId()).asOrderedText(), 
							I18N.translateAsText("oscil.gui.chinf.2", ch.getDimensionId()).asOrderedText(), 
							I18N.translateAsText("oscil.gui.chinf.x", ch.getPos().getX()).asOrderedText(), 
							I18N.translateAsText("oscil.gui.chinf.y", ch.getPos().getY()).asOrderedText(), 
							I18N.translateAsText("oscil.gui.chinf.z", ch.getPos().getZ()).asOrderedText()
					);
				}
				...
				if (deltaX >= 71 && mouseX <= this.active.getX() && deltaY >= 2 && deltaY <= 19) {
					ChannelList.this.tooltip = Lists.newArrayList(
							I18N.translateAsText("oscil.gui.show").asOrderedText()
					);
				}
				
				if (mouseX >= this.active.getX() && deltaY >= 2 && deltaY <= 19) {
					ChannelList.this.tooltip = Lists.newArrayList(
							I18N.translateAsText("oscil.gui.active").asOrderedText()
					);
				}
				
				if (ChannelList.this.tooltip != null) {
					OscilscopeScreen.this.setTooltip(ChannelList.this.tooltip);
				}
			}
	...
	private final class TrigHistory extends CustomPositionEntryListWidget<TrigHistory.Entry> {
		...
		private final class Entry extends AlwaysSelectedEntryListWidget.Entry<Entry> {
			...
			public void render(DrawContext ctx, int j, int y, int x, 
				...
				ctx.drawTextWithShadow(OscilscopeScreen.this.textRenderer, 
						text, x, y, mouseY);
			}
