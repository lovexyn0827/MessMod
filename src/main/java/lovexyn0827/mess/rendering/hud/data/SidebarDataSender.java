...
public interface SidebarDataSender extends HudDataSender {
	...
	default boolean addLine(Entity e, String fieldName, String name, TickingPhase phase, AccessingPath path) {
		...
		boolean canReplace = OptionManager.autoReplaceEntitySidebarLine.orElse(OptionManager.autoReplaceNamedObject);
		if (canReplace) {
			return this.addOrReplaceCustomLine(new SidebarLine(new ListenedField(f, path, name), e, phase));
		} else {
