...
public class LogPacketCommand {
	...
	private static final Object2LongMap<Identifier> STATS = Object2LongMaps.synchronize(new Object2LongOpenHashMap<>());
				...
				.then(literal("stats")
						...
						.executes((ct) -> {
							STATS.forEach((clz, count) -> {
								if (count != 0) {
									String className = clz.getPath();
									CommandUtil.feedbackRawWithArgs(ct, "%s: %d", className, count);
								}
							});
							return Command.SINGLE_SUCCESS;
						})
						.then(argument("types", FilteredSetArgumentType.of(PACKET_TYPES, (o) -> o.getPath()))
								.executes((ct) -> {
									FilteredSetArgumentType.<String>getFiltered(ct, "types").forEach((id) -> {
										long count = STATS.getOrDefault(id, 0);
										if (count != 0) {
											CommandUtil.feedbackRawWithArgs(ct, "%s: %d", id, count);
										}
									});
									return Command.SINGLE_SUCCESS;
								})));
