						...
						.then(argument("corner1", ColumnPosArgumentType.columnPos())
								.executes((ct) -> {
									...
									((ServerWorldAccessor) ct.getSource().getWorld()).getEntityManager()
											.updateTrackingStatus(new ChunkPos(pos.x() >> 4, pos.z() >>4), ChunkLevelType.BLOCK_TICKING);
								...
								.then(argument("corner2", ColumnPosArgumentType.columnPos())
										.executes((ct) -> {
											...
											forEachSelected(ct, (pos) -> {
												posSet.add(pos);
												((ServerWorldAccessor) ct.getSource().getWorld()).getEntityManager()
														.updateTrackingStatus(new ChunkPos(pos), ChunkLevelType.BLOCK_TICKING);
											});
											CommandUtil.feedbackWithArgs(ct, "cmd.general.success");
											return Command.SINGLE_SUCCESS;
										}))))
				.then(literal("remove")
						.then(argument("corner1", ColumnPosArgumentType.columnPos())
								.executes((ct) -> {
									...
									ChunkLevelType type = ((ThreadedAnvilChunkStorageAccessor) ct.getSource().getWorld()
											.getChunkManager().chunkLoadingManager).getCH(posL).getLevelType();
									((ServerWorldAccessor) ct.getSource().getWorld()).getEntityManager()
